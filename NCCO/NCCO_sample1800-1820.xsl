...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <html>
            <head>
                <title>Nineteenth Century Collections Online Sample From 1800-1820</title>
            </head>
            <body>
                <h1>Nineteenth Century Collections Online Sample From 1800-1820</h1>
                <table>
                    <tr>
                        <th>Author</th>
                        <th>Title</th>
    ...
    <xsl:template match="bibl">
        <xsl:for-each select=".">
            <tr>
                ...
                <td><xsl:choose>
                    <xsl:when test=".//title[@level='short'] and .//title[@level='extended']"><i><xsl:apply-templates select=".//title[@level='short']"/></i></xsl:when>
                    <xsl:when test=".//title[@level='short'] and .//title[@level='article']"><xsl:text>"</xsl:text><xsl:apply-templates select=".//title[@level='short']"/><xsl:text>"</xsl:text><br/>
                        <i><xsl:apply-templates select=".//title[@level='journal']"/></i></xsl:when>
                    <xsl:when test=".//title[@level='article'] and .//title[@level='journal']">                
                    <xsl:text>"</xsl:text><xsl:apply-templates select=".//title[@level='article']"/><xsl:text>"</xsl:text><br/>
                    <i><xsl:apply-templates select=".//title[@level='journal']"/></i>
                    </xsl:when>
                    <xsl:otherwise>
